...
void tick() {
  ...
  // 1. Temperature: check every 2 seconds, upload ONLY if temperature value changed
  if (everyMs(s_lastTempUploadMs, 2000)) {
    SystemStatus st = SharedState::snapshot();
  ...
  // 2. Humidity & Smoke: scanned every 5 seconds
  // Smoke is uploaded ONLY if the value changes by >= 50 (or initial upload)
  if (everyMs(s_last5sUploadMs, 5000)) {
    SystemStatus st = SharedState::snapshot();
    FirebaseJson json;
    json.set("humidity", isnan(st.humidity) ? 0.0f : st.humidity);
